...
- 🔧 M3.5 fix-forward — auto-trigger verdict generation after intake submit. The wizard's Submit-on-step-7 was redirecting to the streaming UI in `idle` phase, which forced a second "Generate verdict" click. New `autoStart` prop on `<StreamingVerdict>` driven by `?auto=1` query param fires generation on mount and `router.replace`s the URL to strip the param so refresh-during-stream falls back to the manual click path (preserves the M3.2 cost-control invariant against double-spending). No schema changes.
...
After M3.8 ships, every new verdict's `score_breakdown` JSONB carries the thesis-aware shape (each row has `category`, `weight`, and `multiplier` when a regional override fired). This query surfaces recent breakdowns alongside the property thesis so you can sanity-check that LTR/OO/STR/HH/flipping verdicts are emitting the rules calibrated for them.
...
SELECT
  ...
  v.signal,
  v.confidence,
  jsonb_pretty(v.score_breakdown) AS breakdown
FROM verdicts v
...
A healthy LTR breakdown emits `schools_quality` + `regulatory_thesis` and skips `regulatory_str`; a healthy OO breakdown emits `livability_score` + `appreciation_potential` and skips `cap_rate_vs_price`; a CA verdict's `wildfire` row carries `multiplier: 1.5`. Rows missing the new fields are pre-M3.8 (UI shows the legacy banner; SQL surfaces them by `breakdown -> 0 ->> 'category' IS NULL`).
